...
impl Debug for TicketNewError {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            TicketNewError::TitleError(title) => f.write_str(title),
            TicketNewError::DescriptionError(desc) => f.write_str(desc),
        }
    }
}
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        Err(TicketNewError::TitleError(t)) => panic!("{}",t),
        Err(TicketNewError::DescriptionError(desc)) => Ticket::new(title, "Description not provided".to_string(), status).unwrap(),
        Ok(ticket) => ticket,
    }
}
